...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#070b14] text-slate-100 flex overflow-x-hidden">
      ...
      >
        {/* 🌟 UNIQUE FLOATING BORDER TOGGLE BUTTON */}
        <button
        ...
        <div>
          ...
          {/* Workspace Pill */}
          {!isCollapsed && (
            <div className="mx-3 mt-4 p-2.5 rounded-xl bg-slate-900/60 border border-slate-800/80 flex items-center justify-between">
              <div className="flex items-center gap-2 overflow-hidden">
                <div className="w-6 h-6 rounded-md bg-indigo-500/10 border border-indigo-500/30 flex items-center justify-center text-indigo-400 shrink-0">
        ...
        <div className="p-3 border-t border-slate-800/60">
          <div className={`flex items-center gap-3 ${isCollapsed ? "justify-center" : "px-2 py-1.5"}`}>
            <div className="w-8 h-8 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-sky-400 font-black text-xs shrink-0">
              AI
            </div>
            {!isCollapsed && (
              <div className="flex flex-col overflow-hidden">
