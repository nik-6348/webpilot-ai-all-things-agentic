...
gcloud run deploy webpilot-api \
  ...
  --add-cloudsql-instances "$SQL_CONN" \
  --set-secrets DATABASE_URL="$DB_SECRET:latest" \
  --set-env-vars "GOOGLE_CLOUD_PROJECT=$PROJECT_ID,FIREBASE_PROJECT_ID=$PROJECT_ID,WORKER_URL=$WORKER_URL,WORKER_AUDIENCE=$WORKER_URL,TASK_QUEUE=$QUEUE,TASK_LOCATION=$REGION,TASK_INVOKER_SA=$TASK_SA,SCHEDULER_INVOKER_SA=$SCHED_SA,LOCAL_TASKS=false,LOCAL_SECRETS=false,LOCAL_SCHEDULER=false,LOCAL_PUBSUB=false,LOCAL_AUTH_BYPASS=false,ALLOW_PRIVATE_DEMO=false,RESEND_API_KEY=${RESEND_API_KEY:-}" \
