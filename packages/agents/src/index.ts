...
export async function planWorkflow(input:{goal:string;targetUrl:string;allowedDomains:string[];credentialFields?:string[]}){
  if(process.env.MOCK_AI === "true") return PlanSchema.parse({summary:"Monitor supplier purchase-order exceptions",requiresApproval:true,workflow:{version:1,goal:input.goal,startUrl:input.targetUrl,allowedDomains:input.allowedDomains,extractionSchema:{recordLocator:{strategy:"css",value:".order-row"},fields:[{name:"id",type:"string",required:true,locator:{strategy:"css",value:".order-id"}},{name:"supplier",type:"string",required:true,locator:{strategy:"css",value:".supplier"}},{name:"status",type:"string",required:true,locator:{strategy:"css",value:".status"}},{name:"eta",type:"date",required:true,locator:{strategy:"css",value:".eta"}},{name:"amount",type:"string",required:true,locator:{strategy:"css",value:".amount"}}]},steps:[]}});
...
export async function navigateDiscovery(input:{goal:string;schema:unknown;url:string;dom:string;history:unknown[];screenshotBase64?:string}){
  if(process.env.MOCK_AI === "true"){const h=input.history as any[];if(h.length===0)return BrowserDecisionSchema.parse({action:{id:"next",type:"CLICK",description:"Open purchase orders",locator:{strategy:"text",value:"View Orders"},risk:"LOW"},done:false,rationale:"Open orders"});return BrowserDecisionSchema.parse({action:{id:"next",type:"EXTRACT",description:"Extract purchase orders",risk:"LOW"},done:true,rationale:"Orders are visible"});}
...
export async function recoverWorkflow(input:{goal:string;workflow:unknown;failedStep:any;error:string;url:string;dom:string;screenshotBase64?:string}){
  if(process.env.MOCK_AI === "true")return WorkflowPatchSchema.parse({failedStepId:input.failedStep.id,replacement:{...input.failedStep,locator:{strategy:"testId",value:"orders-action"}},diagnosis:"The portal renamed and moved the orders action",confidence:.99,risk:"LOW"});
...
export async function verifyRecovery(input:unknown){ if(process.env.MOCK_AI === "true")return VerificationSchema.parse({verdict:(input as any)?.verification?.passed?"PASS":"FAIL",reason:(input as any)?.verification?.passed?"Replay passed":"Replay failed",confidence:.99}); return runJson("verifier_agent","Independently verify a proposed recovery using replay evidence. The repair agent cannot approve itself.",VerificationSchema,[{text:JSON.stringify(input)}]); }
